...
class SendPlatformPasswordResetEmailJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(SenderAliasResolver $aliases, MailTransportConfigurator $mailTransport): void
    {
        ...
        // El enlace de restablecimiento ya viene con vencimiento propio. Si esa
        // ventana pasó, no tiene sentido enviar el correo (el enlace estaría
        // muerto), pase lo que pase con el transporte SMTP. Se marca fallido
        // explícitamente para que quede visible en el panel.
        $windowMinutes = max(1, (int) data_get($message->metadata, 'expires_minutes', 60));

        if ($message->created_at?->copy()->addMinutes($windowMinutes)->isPast()) {
            $message->forceFill([
                'status' => 'failed',
                'last_error' => 'El enlace de restablecimiento expiró antes de poder enviar el correo.',
            ])->save();
            $message->recordEvent('failed', ['reason' => 'reset_link_expired']);
        ...
        try {
            ...
            if (! $activeTransport) {
                ...
                // Re-encolamos una copia fresca (no release) para que la espera por
                // falta de transporte no consuma el presupuesto de $tries reservado
                // a los fallos reales de envío. El guard de vencimiento de arriba
                // corta estos reintentos cuando el enlace caduca.
                static::dispatch($this->messageId)->delay(now()->addMinutes(5));
            ...
            Mail::to($message->recipient_email, $message->recipient_name)
                ->send(new PlatformPasswordResetMail($message->fresh()));
    ...
    private function resolveSenderAlias(NotificationMessage $message, SenderAliasResolver $aliases): void
    {
        ...
        $alias = $aliases->resolve($message->purpose ?: 'platform_password_reset');
